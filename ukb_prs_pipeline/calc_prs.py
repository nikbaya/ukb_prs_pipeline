...
def get_prs(prs_method, beta_est_path, dataset, out):
    ...
    if prs_method == "prs_cs":
        beta_est = hl.import_table(
            paths=[f'{beta_est_path}_chr{chrom}' for chrom in AUTOSOMES],
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--get_prs', default=False, action="store_true", help='Whether to get PRS')
    parser.add_argument('--prs_method', default=None, help='PRS method used to get estimated effect sizes')
    parser.add_argument('--beta_est_path', default=None, help='Path to estimated effect sizes')
    parser.add_argument('--dataset', default=None, help='Which dataset to use when calculating PRS')
    parser.add_argument('--out', default=None, help='Path to write output file')
